...
def annotate_music21_score(score, n_measures=100, start_measure=1, depth=0, rbeam=0, lbeam=1, quiet=False, below_beam=False, left_only=False, right_only=False, hand_size_XXS=False, hand_size_XS=False, hand_size_S=False, hand_size_M=False, hand_size_L=False, hand_size_XL=True, hand_size_XXL=False):
    """
    Annotates a music21 score with fingering annotations.

    Args:
        score (music21.stream.Score): The music21 score to annotate.
        n_measures (int): Number of measures to process.
        start_measure (int): Starting measure.
        depth (int): Depth for hand generation.
        rbeam (int): Right hand part index.
        lbeam (int): Left hand part index.
        quiet (bool): Suppress output.
        below_beam (bool): Place lyrics below beam.
        left_only (bool): Process left hand only.
        right_only (bool): Process right hand only.
        hand_size_XXS (bool): Set hand size to XXS.
        hand_size_XS (bool): Set hand size to XS.
        hand_size_S (bool): Set hand size to S.
        hand_size_M (bool): Set hand size to M.
        hand_size_L (bool): Set hand size to L.
        hand_size_XL (bool): Set hand size to XL.
        hand_size_XXL (bool): Set hand size to XXL.

    Returns:
        music21.stream.Score: The annotated music21 score.
    """
    hand_size = 'M'  # default
    if hand_size_XXS: hand_size = 'XXS'
    if hand_size_XS: hand_size = 'XS'
    if hand_size_S: hand_size = 'S'
    if hand_size_M: hand_size = 'M'
    if hand_size_L: hand_size = 'L'
    if hand_size_XL: hand_size = 'XL'
    if hand_size_XXL: hand_size = 'XXL'

    if not left_only:
        rh_noteseq = reader(score, beam=rbeam)
        rh = Hand(side="right", noteseq=rh_noteseq, size=hand_size)
        ...
        if depth == 0:
            ...
            rh.autodepth = False
            rh.depth = depth
        rh.lyrics = below_beam
        rh.generate(start_measure, n_measures)
    if not right_only:
        lh_noteseq = reader(score, beam=lbeam)
        lh = Hand(side="left", noteseq=lh_noteseq, size=hand_size)
        ...
        if depth == 0:
            ...
            lh.autodepth = False
            lh.depth = depth
        lh.lyrics = below_beam
        lh.noteseq = lh_noteseq
        lh.generate(start_measure, n_measures)

    if not left_only:
        score = annotate_fingers_xml(score, rh, Args(rbeam = rbeam, lbeam = lbeam), is_right=True)
    if not right_only:
        score = annotate_fingers_xml(score, lh, Args(rbeam = rbeam, lbeam = lbeam), is_right=False)
